...
def jit_prefill(state, time_step, input_tokens):
...
# Here, you can change the partition spec of the model weights and KV cache, by
# using input_shardings.
@functools.partial(jax.jit, donate_argnames=("kv_state",))
def jit_decode(state, kv_state, input_tokens):
    # Here, you don't have to use `inference_runner.model.decoder`. You can instantiate from
    # a modified model_cfg with potentially different partition spec and use that instead, e.g.
    module = model_cfg.set(name="new_pspec_model").instantiate(parent=None).decoder
    # module = inference_runner.model.decoder
    (kv_state, outputs), _ = F(
        module=module,
        prng_key=prng_key,
        state=state,
        inputs={"cached_states": kv_state, "input_ids": input_tokens},
        method="extend_step",
        is_training=False,
    )
    return kv_state, outputs


with inference_runner._mesh:
    model_state = inference_runner._inference_runner_state.model["decoder"]
    kv_cache_states, _ = jit_prefill(model_state, time_step, input_tokens)

# Here, it's possible to use a different mesh also.
with inference_runner._mesh:
    for i in range(4):
        decode_tokens = jnp.ones((32, 1), dtype=jnp.int32)
        kv_cache_states, _ = jit_decode(model_state, kv_cache_states, decode_tokens)
