...
class TestFlashAttention(TestCase):
    ...
    @parameterized.product(seq_len=[8, 16, 32, 128], left_context=[4, 8, 16])
    def test_sliding_window_mask_equivalence(self, seq_len, left_context):
        shape = (seq_len, seq_len)
        ref_mask = splash_attention_mask.LocalMask(
            shape=shape, window_size=(left_context, 0), offset=0
        )

        mask_fn = sliding_window_causal_mask(left_context=left_context)
    ...
    def test_forward_and_backward(
        ...
        if attention_bias is not None:
            composite_bias.append(TensorAttentionBias(attention_bias))
        if segment_ids is not None:
            composite_bias.append(SegmentIdAttentionBias(segment_ids))
        composite_bias = CompositeAttentionBias(composite_bias)

        def fn(q, k, v, bias):
            specs = flash_attention_implementation(
                query=q,
                key=k,
                value=v,
                bias=bias,
            ...
            if fallback_to_legacy:
                self.assertIn("jit_attn", specs.fn.__qualname__)
            return specs.fn(q, k, v, bias, None, *specs.additional_args)

        # Compare outputs.
        out = fn(q, k, v, composite_bias)
