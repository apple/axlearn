    ...
    def extend_step(
        ...
        *,
        ...
        Args:
            ...
            proj_positions: An optional Tensor of shape [batch, source_length].

        Returns:
            A tuple (updated_state, output):
            * updated_state: A `Nested[Tensor]` object containing KV cache such as key and value.
            * output: The output k_proj, v_proj, and key_positions will be used in the current
                attention operation.
        """
        raise NotImplementedError(type(self))
...
class KVCache(BaseKVCache):
    ...
    def extend_step(
        ...
        *,
        ...
        if k_proj.shape[1] != proj_positions.shape[1]:
            raise ValueError(f"{k_proj.shape[1]=} != {proj_positions.shape[1]=}")
        ...
        oh_indices = jax.nn.one_hot(proj_positions, source_len, dtype=cached_key.dtype)
            ...
            if not kv_sharing:
                ...
                with child_context("kv_cache_extend_step", module=self.kv_cache):
                    new_cached_states["kv_cache"], kv_cache_output = self.kv_cache.extend_step(
                        ...
                        k_proj=k_proj,
                        v_proj=v_proj,
                        proj_positions=query_positions,
                    )
