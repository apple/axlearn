...
class BaseKVCache(BaseLayer):
    ...
    class Output(NamedTuple):
        ...
        Fields:
            ...
            query_positions: [batch, steps], Positions of the queries in the batch.
    ...
    def extend_step(
        self, cached_states: Nested[Tensor], *, k_proj: Tensor, v_proj: Tensor, time_step: Tensor
        ...
        Args:
            ...
            k_proj: A Tensor of shape [batch, source_length, num_kv_heads, per_head_dim].
            v_proj: A Tensor of shape [batch, source_length, num_heads, per_head_dim].
            time_step: A Tensor of shape [batch]. Each value is an index of the KV cache.

        Returns:
            A tuple (updated_state, output):
            * updated_state: A `Nested[Tensor]` object containing KV cache such as key and value.
            * output: The output k_proj, v_proj, query_positions and key_positions will be used
                in the current attention operation.
        """
        raise NotImplementedError("")
...
class KVCache(BaseKVCache):
    ...
    def extend_step(
        ...
        *,
        ...
        step_size = k_proj.shape[1]
        query_positions = jnp.arange(step_size)[None] + time_step[:, None]  # [batch, steps]
...
class SlidingWindowKVCache(BaseKVCache):
    ...
    def extend_step(
        ...
        *,
        ...
        step_len = k_proj.shape[1]

        k_proj, _ = einops.pack([cached_key, k_proj], "b * n d")
        v_proj, _ = einops.pack([cached_value, v_proj], "b * n d")
        updated_state = dict(key=k_proj[:, step_len:], value=v_proj[:, step_len:])
        ...
        # [batch, steps]
        key_positions = (
            jnp.arange(k_proj.shape[1])[None] + time_step[:, None] - cfg.cached_kv_length
        )
        return updated_state, self.Output(
            k_proj=k_proj,
            v_proj=v_proj,
...
class BaseQKVLinear(BaseLayer):
    ...
    def init_states(
        ...
        * To prefill, provide `time_step` and `query` as Tensors.
        ...
        if kv_state is None:
            kv_shape = dict(num_kv_heads=self.num_kv_heads, per_head_dim=cfg.per_head_dim)
            if key is None:
                kv_shape.update(batch_size=query.shape[0], kv_len=query.shape[1])
            else:
                chex.assert_equal_shape((key, value))
                kv_shape.update(batch_size=key.shape[0], kv_len=key.shape[1])
            kv_shape = KVCache.Shape(**kv_shape)
            if query.dtype is None:
                raise ValueError("init_states dtype is not set.")
            init_state.update(self.kv_cache.init_states(shape=kv_shape, dtype=query.dtype))
        ...
        if time_step is not None:
            ...
            # `extend_step` updates `time_step``, but there are cases where it uses a shorter
            # `time_step`` than the cached key.
            # TODO(dhwang2): This results in wasted attention computation. Ensure that all callers
            # pass keys of the same length as time_step. Once that is fixed, time_step can be
            # inferred from the key length, so it should be removed from the API.
    ...
    def extend_step(
        ...
        *,
        ...
        q_proj, k_proj, v_proj, unused_q_pos, unused_k_pos = self.forward(
            query, query_positions=query_positions, **kv_kwargs
        )
        updated_state = dict(time_step=time_step + num_query_steps)
        if kv_state is None:
            with child_context("kv_cache_extend_step", module=self.kv_cache):
                kv_cache, output = self.kv_cache.extend_step(
                    cached_states, k_proj=k_proj, v_proj=v_proj, time_step=time_step
                )
            updated_state.update(kv_cache)
            k_proj, v_proj, query_positions, key_positions = (
                output.k_proj,
                output.v_proj,
                output.query_positions,
                output.key_positions,
            )
        else:
...
class QKVLinear(BaseQKVLinear):
    ...
    def forward(
        ...
        *,
        ...
        key_positions = query_positions if key is None else jnp.arange(key.shape[1])[None]
        return self.Output(
            query=q_proj,
            key=k_proj,
            value=v_proj,
...
class QLinear(BaseQKVLinear):
    ...
    def forward(
        ...
        *,
        ...
        if query_positions is None:
            query_positions = jnp.arange(q_proj.shape[1])[None]
        key_positions = jnp.arange(kv_state.k_proj.shape[1])[None]
        return self.Output(
            query=q_proj,
            key=kv_state.k_proj,
            value=kv_state.v_proj,
            query_positions=query_positions,
            key_positions=key_positions,
        )
...
class FusedQKVLinear(BaseQKVLinear):
    ...
    def initialize_parameters_recursively(
        ...
        def init(prng_key_i):
            return VDict(
                qkv_proj=self.qkv_proj.initialize_parameters_recursively(prng_key_i),
                kv_cache=dict(),
            )
...
class RoFormerQKVLinear(BaseQKVLinear):
    ...
    def forward(
        ...
        *,
        ...
        query, key, value, query_positions, key_positions = self.i_proj(
...
class MultiheadAttention(BaseLayer):
    ...
    def __init__(self, cfg: Config, *, parent: Module):
        ...
        if cfg.causal:
            ...
            # For backward compatibility, but mask=causal_mask is DEPRECATED.
        ...
        if self._mask_tpl is not None:
            if not isinstance(self._mask_tpl, ClassConfigBase) or not issubclass(
                self._mask_tpl.klass, MaskFnAttentionBias
            ):
                raise ValueError(
                    f"{self._mask_tpl=} must be InstantiableConfig[MaskFnAttentionBias]."
                )
    ...
    def _forward_for_mode(
        ...
        *,
        ...
        q_proj, k_proj, v_proj, query_positions, key_positions = i_proj_output
        ...
        if self._mask_tpl is not None:
            attention_logit_biases += self._mask(
                target_positions=query_positions,
                source_positions=key_positions,
                dtype=q_proj.dtype,
            )
    ...
    def _mask(
        ...
        *,
        ...
        """Returns an attention logit biases, based on the template.

        Args:
            target_positions: The positions that the mask should be computed for,
                [batch_size, target_length].
            source_positions: The positions that the mask should be computed for,
                [batch_size, target_length].
            dtype: the dtype of the biases.

        Returns:
            An attention bias represented as an implicit boolean mask.
        """
        mask_tpl = self._mask_tpl
        # We cannot use mask_tpl.initialize() because Config cannot have a Tensor as an attribute.
        klass = mask_tpl.klass
        kwargs = {k: v for k, v in mask_tpl.items() if k != "klass"}
        kwargs.update(
            target_positions=target_positions, source_positions=source_positions, dtype=dtype
        )
        return klass(**kwargs)
...
class SlidingWindowAttentionModifier(ConfigModifier):
    """Converts self-attention to sliding window attention."""

    @config_class
    class Config(ConfigModifier.Config):
        """Configure SlidingWindowAttentionModifier."""

        left_context: Required[int] = REQUIRED

    def __call__(self, cfg: MultiheadAttention.Config) -> MultiheadAttention.Config:
        """Enable sliding window attention.

        Ensure that the KV cache and attention mask operate with sliding window attention in mind.

        Args:
            cfg: MultiheadAttention Config.

        Returns:
            A new MultiheadAttention Config with sliding window attention enabled.
        """
        cfg = cfg.clone()
        left_context = self.config.left_context
        if cfg.input_linear.kv_cache is not None:
            kv_kwargs = dict(cfg.input_linear.kv_cache.items())
            kv_kwargs["klass"] = SlidingWindowKVCache
            kv_cache = SlidingWindowKVCache.Config(**kv_kwargs)
        else:
            kv_cache = SlidingWindowKVCache.default_config()
        kv_cache.set(cached_kv_length=left_context)
        cfg.input_linear.set(kv_cache=kv_cache)
        if "input_linear" in cfg.input_linear:
            cfg.input_linear.input_linear.set(kv_cache=kv_cache)
        mask = SlidingWindowAttentionBias.default_config(left_context=left_context)
        cfg.set(mask=mask)
        return cfg


def enable_sliding_window_attention(
    cfg: MultiheadAttention.Config, left_context: int
) -> MultiheadAttention.Config:
    """Enable sliding window attention.

    Args:
        cfg: MultiheadAttention Config.
        left_context: sliding window size.

    Returns:
        A new MultiheadAttention Config with sliding window attention enabled.
    """
    modifier = (
        SlidingWindowAttentionModifier.default_config().set(left_context=left_context).instantiate()
    )
    return modifier(cfg)
