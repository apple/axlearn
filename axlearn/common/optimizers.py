...
def composite(
    optimizer_rules: Sequence[SubOptimizerRule],
) -> PartitionedGradientTransformation:
    """Applies different optimizers on different subset of parameters.

    It wraps around optax.multi_transform and adds partition function.
    https://github.com/deepmind/optax/blob/84c644939d0ffe161403607a40996093e0b6c214/optax/_src/combine.py#L75.

    Args:
        optimizer_rules: a sequence of sub-optimizer rule. The first param_regex
            pattern that fully matches the parameter path (separated by "/") determines
            the optimizer for the parameter. All parameters must match at least one rule.

    Returns:
        A PartitionedGradientTransformation.

    Raises:
        ValueError: If values in optimizer_by_param_path is not a
            PartitionedGradientTransformation.
        ValueError: If rules a parameter does not match with any rule.
    """
    # A mapping from rule key to optimizer.
    transforms = {}
    for rule in optimizer_rules:
        transforms[rule.param_regex] = _to_partitioned_transformation(rule.optimizer)

    def param_rule_fn(params):
        """Returns a tree of the same structure as the params where each leaf is the rule key."""

        def per_param_fn(param_path: str):
            # TODO(ruoming): use `match_regex_rules`.
            for rule in optimizer_rules:
                if re.fullmatch(rule.param_regex, param_path):
                    logging.info(
                        "Composite optimizer: %s matches %s.", rule.param_regex, param_path
                    )
                    return rule.param_regex
            raise ValueError(f"Composite optimizer: {param_path} does not match any optimizer.")

        return jax.tree_util.tree_map(per_param_fn, tree_paths(params, separator="/"))

    # optax.multi_transform returns a composite optimizer, given `transforms` and `param_labels`.
    # `transforms` is a mapping from keys to gradient transformations. `param_labels` is a PyTree
    # or a callable that generates the tree. The tree is of same structure as parameters/updates,
    # with leaves being the key of the transforms to apply.
    base = optax.multi_transform(
        transforms={
            k: optax.GradientTransformation(init=t.init, update=t.update)
            for k, t in transforms.items()
        },
        param_labels=param_rule_fn,
    )

    def partition(param_spec):
        param_rules = param_rule_fn(param_spec)
        # A mapping from rule to sub-optimizer states of type optax.MaskedState.
        # For sub-optimizer state, if it is a tree of per parameter state, only
        # the subset of parameters that belongs to the optimizer is kept and
        # the rest is masked as a optax.MaskNode().
        inner_states = {}
        for rule, tx in transforms.items():
            # Keep only the param_specs that belong to the sub-optimizer.
            masked_specs = jax.tree_util.tree_map(
                lambda key, spec, r=rule: spec if key == r else optax.MaskedNode(),
                param_rules,
                param_spec,
            )
            inner_states[rule] = optax.MaskedState(inner_state=tx.partition(masked_specs))
        # Use MultiTransformState to be consistent with the states maintained by
