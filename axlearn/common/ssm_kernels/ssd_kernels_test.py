...
def _ssd_naive_reference(q, k, v, log_alpha, h0=None):
    '''
    For-loop reference implementation of SSD. Note that this implementation somehow have worse
...
def ssd_chunk_tri(X, A, B, C, chunk_size=16, initial_states=None):
    ...
    Tri's implementation from https://github.com/state-spaces/mamba/blob/main/mamba_ssm/modules/ssd_minimal.py
    X, A, B, C corresponds to V, \alpha, K, Q in linear attention (H_t = \alpha H_{t-1)+ K_t^\top V_t, O_t = Q_t S_t)
    """
