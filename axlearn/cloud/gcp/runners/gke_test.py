...
from axlearn.cloud.gcp.job_flink import FlinkJobStates
...
class FlinkGKERunnerJobTest(parameterized.TestCase):
    ...
    @parameterized.parameters(
        dict(
            flink_response={"jobs": [{"state": FlinkJobStates.FINISHED.value}]},
            expected=runner_gke.GKERunnerJob.Status.SUCCEEDED,
        ),
        dict(
            flink_response={"jobs": [{"state": FlinkJobStates.FAILED.value}]},
            expected=runner_gke.GKERunnerJob.Status.FAILED,
        ),
        dict(
            flink_response={"jobs": [{"state": FlinkJobStates.FAILING.value}]},
            expected=runner_gke.GKERunnerJob.Status.FAILED,
        ),
        dict(
    ...
    def test_get_flink_job_status(self, mock_get, flink_response, expected):
        ...
        job._inner.jobmanager_ip = "127.0.0.1"
    ...
    def test_get_flink_job_status_raises_on_request_error(self, mock_get):
        ...
        self.assertIn("error getting flink job satus", str(context.exception))
