...
def _bbox_overlap(boxes, gt_boxes):
    ...
    """
    bb_y_min, bb_x_min, bb_y_max, bb_x_max = jnp.split(ary=boxes, indices_or_sections=4, axis=2)
    gt_y_min, gt_x_min, gt_y_max, gt_x_max = jnp.split(ary=gt_boxes, indices_or_sections=4, axis=2)
...
def _self_suppression(in_args):
    ...
    can_suppress_others = jnp.reshape(jnp.max(iou, 1) <= 0.5, [batch_size, -1, 1]).astype(iou.dtype)
    iou_suppressed = (
        jnp.reshape(
            (jnp.max(can_suppress_others * iou, 1) <= 0.5).astype(iou.dtype), [batch_size, -1, 1]
        )
        * iou
    )
...
def _cross_suppression(in_args):
    ...
    ret_slice = (
        jnp.expand_dims((jnp.all(iou < iou_threshold, [1])).astype(box_slice.dtype), 2) * box_slice
    )
...
def non_max_suppression_padded(
    scores: Tensor, boxes: Tensor, max_output_size: Tensor, iou_threshold: float
):
    ...
    pad = int(np.ceil(float(num_boxes) / _NMS_TILE_SIZE)) * _NMS_TILE_SIZE - num_boxes
    ...
    idx = jnp.reshape(idx + jnp.reshape(jnp.arange(batch_size) * num_boxes, [-1, 1]), [-1])
    boxes = jnp.reshape((jnp.reshape(boxes, [-1, 4]))[idx], [batch_size, max_output_size, 4])
    boxes = boxes * (
        jnp.reshape(jnp.arange(max_output_size), [1, -1, 1]) < jnp.reshape(output_size, [-1, 1, 1])
    ).astype(boxes.dtype)
    scores = jnp.reshape(jnp.reshape(scores, [-1, 1])[idx], [batch_size, max_output_size])
    scores = scores * (
        jnp.reshape(jnp.arange(max_output_size), [1, -1]) < jnp.reshape(output_size, [-1, 1])
    ).astype(scores.dtype)
    return scores, boxes
