...
from axlearn.gcp.config import gcp_settings
from axlearn.gcp.docker import registry_from_repo
from axlearn.gcp.storage import list_blobs
from axlearn.gcp.utils import format_table, is_valid_resource_name
    ...
    while True:
        ...
        if node is not None:  # TPU exists.
            ...
            if state == "READY":
                ...
                total_vms = int(tpu_type.split("-")[-1]) // 8
    ...
    *,
    ...
    while True:
        ...
        if state in ["ACCEPTED", "PROVISIONING"]:
            ...
            num_vms = num_slices * int(tpu_type.split("-")[-1]) // 8
            ready_flags_base_path = (
                f"gs://{gcp_settings('ttl_bucket')}/axlearn/tasks/{name}/tpu_vm_ready_flags"
            )
...
def get_tpu_node(name: str, resource: discovery.Resource) -> Optional[Dict[str, Any]]:
    ...
    up_nodes = resource.list(
        parent=f"projects/{gcp_settings('project')}/locations/{gcp_settings('zone')}"
    ).execute()
    nodes = [el for el in up_nodes.get("nodes", {}) if el.get("name", "").split("/")[-1] == name]
