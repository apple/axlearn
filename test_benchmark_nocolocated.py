...
def main():
    ...
    parser.add_argument("--enable_pathways", required=False,
                       help="set it to True if needed (e.g., True or False")
    
    args = parser.parse_args()

    if bool(args.enable_pathways):
        import pathwaysutils
    ...
    with create_mesh():
        ...
        data_gb_per_device = total_data_moved_gb / len(shardings)

        throughput_gb_tot = total_data_moved_gb / (transfer_time+preload_time)
        ...
        print(f"Aggregated GCS -> Devices using colocated Throughput: {throughput_gb_tot:.2f} GiB/s")
