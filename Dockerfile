...
RUN uv pip install .[core,dev,grain,gcp,vertexai_tensorboard,open_api] && uv cache clean && echo "=== BEGIN pip freeze" && uv pip freeze && echo "=== END pip freeze" && echo "=== BEGIN pipdeptree" && pipdeptree && echo "=== END pipdeptree"
COPY . .
